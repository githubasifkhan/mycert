...
# Booking From
class PropertyBookingSales(models.Model):
	...
	def unlink(self):
		...
		return super(PropertyBookingSales,self).unlink()

	@api.model
	def create(self, vals):
		vals['name']  = self.env['ir.sequence'].next_by_code('property.booking.sales') or 'New'
		return super(PropertyBookingSales,self).create(vals)
	...
	landlord = fields.Many2many('res.partner',string="Landlord",compute="_get_landlord",store=True)
	...
	unit_ids = fields.Many2many('property.unit', 'rel_booking_unit_sales','booking_id','unit_id',string="Units", required=True)
	...
	def _get_landlord(self):
		for rec in self:
			lord = []
			for line in rec.build_id.main_property_id.ownership:
				lord.append(line.name.id)
			rec.landlord = [(6,0,lord)]
	...
	def action_booking_send(self):
		...
		try:
			...
			template_id = ir_model_data.get_object_reference('ag_property_maintainence', 'email_template_edi_booking_sales_done')[1]
		except ValueError:
			template_id = False
		try:
		...
		ctx.update({
		'default_model': 'property.booking.sales',
		'active_model': 'property.booking.sales',
		...
		# else:
		ctx['model_description'] = _('Sales Booking')
...
class PropertyContractSales(models.Model):
	...
	@api.model
	def create(self, vals):
		res = super(PropertyContractSales, self).create(vals)
		if res:
...
# 			next_date = str(self.get_add_month(next_date))[:10]
	...
	def unlink(self):
		...
		return super(PropertyContractSales,self).unlink()
...
# 		return True
	...
	#
	@api.depends('unit_line.vat_amount_total')
	...
	renew_id = fields.Many2one('property.contract.sales', string="Renew")
	...
	# date_start = fields.Date(string="Date Strat", required=True, help="From D ate", track_visibility='onchange')
	...
	progress = fields.Integer(string="Progress", required=False, default=0)
	user_id = fields.Many2one('res.users', string='Salesman', index=True, track_visibility='onchange', default=lambda self: self.env.user)
	...
	booking_id = fields.Many2one('property.booking.sales',string="Booking")
	...
	unit_line = fields.One2many('property.cont.unit.sales','cont_id',string="Units")
	...
	def action_contract_send(self):
		...
		ir_model_data = self.env['ir.model.data']
		try:
			template_id = ir_model_data.get_object_reference('ag_property_maintainence', 'email_template_edi_contract_sales_done')[1]
		except ValueError:
			template_id = False
		try:
			compose_form_id = ir_model_data.get_object_reference('mail', 'email_compose_message_wizard_form')[1]
		...
		ctx = dict(self.env.context or {})
		ctx.update({
		'default_model': 'property.contract.sales',
		'active_model': 'property.contract.sales',
		'active_id': self.ids[0],
		'default_res_id': self.ids[0],
		'default_use_template': bool(template_id),
		'default_template_id': template_id,
		...
		# 	ctx['model_description'] = _('Request for Quotation')
		# else:
		ctx['model_description'] = _('Sales Contract')
...
# 			self.unit_line = line_vals        		
	...
	def booking_onchange(self):
		...
		for line in self.booking_id.unit_ids:
			...
			dict_vals = (0,0,{'unit_id':unit_id,'unit_rent':0,
	...
	def approve(self):
		...
		name = self.name
		if self.name == 'Draft':
			name = self.env['ir.sequence'].next_by_code('property.contract.sales') or 'Draft'
			self.name = name
		...
		self.unit_line.write({'partner_id':self.customer_id.id})
	...
	def post(self):
		...
		# 	date_start = u_line.cont_id.con_date 
		...
		# 		move_vals = {'journal_id':journal_id,'date':date,'ref': name + _(' :: ') + str(self.customer_id.name)} 				
		# 		if self.is_contract:
		# 			#line1 = (0,0,{'account_id':pdc_account_new_id,'partner_id':partner_id,'analytic_account_id':analytic_id,'name':name,'credit':0.0,'debit':line.amount,'maintain_cc_id':cost_center})
...
# ##############################################################################################################
class PropertyContUnitSales(models.Model):
	_name = 'property.cont.unit.sales'
	...
	@api.model
	def create(self, vals):
		vals['name']  = self.env['ir.sequence'].next_by_code('property.cont.unit.sales') or 'New'
		return super(PropertyContUnitSales,self).create(vals)
	#   
	...
	def onchange_unit(self):
		...
		contract_line_obj = self.env['property.cont.unit.sales']
	...
	list_rent = fields.Float(string="Unit Price", track_visibility='onchange')
	duration = fields.Selection([('mt', 'Monthly'),('yr', 'Yearly'),('sales','Sales')], string='Period', index=True, required=True, default='sales')
	year_rent = fields.Float(string="Total Price", track_visibility='onchange')
	...
	unit_to = fields.Date(string="Date To", required=False, track_visibility='onchange')
	...
	cont_id = fields.Many2one('property.contract.sales', ondelete='cascade', string="Contract")
	...
	#
	@api.depends('list_rent','unit_rent','year_rent','unit_from')
	@api.onchange('list_rent','unit_rent','year_rent','unit_from')
	...
	def unit_onchange(self):
		...
		if len(rent_lines) >0 and self.unit_id:
			line_ids = rent_lines.search([('date_from','<=',dt_from),('unit_id','=',self.unit_id.id)])
	...
	@api.onchange('unit_id','unit_rent','unit_from','year_rent','percent','mth_count','free_unit_mth','unit_sqft','net_area','list_rent','line_disc','duration')
	def onchange_unit_values(self):
		for rec in self:
			rec.year_rent = rec.list_rent
